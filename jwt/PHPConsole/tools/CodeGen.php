...
class CodeGen
{
    ...
    private function getNavigationStateName($navigation)
    {
        ...
        {
            $layout = $this->app->Layouts[$navigation['HasLayout']];
            $nameList[]=$layout['LayoutName'];
            while (!JwtUtil::IsNullOrEmptyString($layout['Extend']))
            {
                $layout =  $this->app->Layouts[$layout['Extend']];
        ...
        //$this->navList[$navigation['NavigationName']] = sprintf("'%s':['%s','%s']", $navigation['NavigationName'], implode("/", array_reverse($nameList)), JwtUtil::IsNullOrEmptyString($navigation['ParamName'])?"":$navigation['ParamName']);
        return implode(".", array_reverse($nameList));
    }
