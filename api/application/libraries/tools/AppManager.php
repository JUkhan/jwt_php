...
class AppManager
{
    ...
    public function GetItemDetail($name, $mode)
    {
     ...
     public function SaveFile($mode, $directoryName, $fileName, $content)
        ...
        {
            if (! JwtUtil::endsWith($fileName, $ext)) $fileName .= ext;
            ...
            $res=new stdClass();
            $res->exist=JwtUtil::fileExists($path);
            return $res;
        ...
        public function AddFile($mode, $directoryName, $fileName, $ext)
        {
            ...
            $this->UpdateAppDirectivesAndCSS();
            $res=new stdClass();
            $res->isSuccess = true;
        ...
        private function UpdateAppDirectivesAndCSS()
        {
            $dir = JwtUtil::getSubDirectories($this->rootPath . "Scripts/Directives");
            $import = new StringBuilder();
            $builder = new StringBuilder();
            $componentsCSS = new StringBuilder();
            foreach ($dir as  $name)
            {
                $import->appendFormat("import %s from 'Scripts/Directives/$s/%s.js';", $name, $name, $name);
                $import->appendLine();

                $builder->appendFormat(".directive('%s', %s.builder)", $name, $name);
                $builder->appendLine();
                if (JwtUtil::fileExists($this->rootPath . "Scripts/Directives/".$name."/".$name.".css"))
                {
                    $componentsCSS->appendFormat("@import '../Scripts/Directives/%s/%s.css';", $name, $name);
                    $componentsCSS->appendLine();
                }
            }
            $dir =JwtUtil::getSubDirectories($this->rootPath . "Scripts/Modules");
            foreach ($dir as  $name)
            {
                if (JwtUtil::fileExists($this->rootPath .  "Scripts/Modules/".$name."/".$name.".css"))
                {
                    $componentsCSS->appendFormat("@import '../Scripts/Modules/%s/%s.css';", $name, $name);
                    $componentsCSS->appendLine();
                }
            }
            $dir =JwtUtil::getSubDirectories($this->rootPath . "Scripts/Components");
           
            foreach ($dir as  $name)
            {              
                if (JwtUtil::fileExists($this->rootPath .  "Scripts/Components/".$name."/".$name.".css"))
                {
                    $componentsCSS->appendFormat("@import '../Scripts/Components/%s/%s.css';", $name, $name);
                    $componentsCSS->appendLine();
                }
            }
            $res = new StringBuilder();
            $res->append($import->toString());
            $res->appendLine();
            $res->appendLine();
            ...
            JwtUtil::putContent($this->rootPath . "Scripts/app.directives.js", $res->toString());
            JwtUtil::putContent($this->rootPath . "Content/components.css", $componentsCSS->toString());
        }
}
